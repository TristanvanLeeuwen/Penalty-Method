...
\]
For $\lambda>\|A^{-T}P^T\|_2^2$ we may expand the inverse as $(I + \lambda^{-1}B)^{-1} \approx I - \lambda^{-1}B + \lambda^{-2}B^2 + \ldots$,
and find that
\bq
...
\begin{remark}
\label{remark}
Lemma \ref{lemma} suggests a natural scaling for the penalty parameter, $\lambda > \|A^{-T}P^T\|_2^2$ can be considered large, while $\lambda < \|A^{-T}P^T\|_2^2$ can be considered small. 
\end{remark}
...
\eq
Using the approximations for $\mbf{u}_{\lambda}$ and $\mbf{v}_{\lambda}$ for $\lambda>\|A^{-T}P^T\|_2^2$ presented in Lemma \ref{lemma}, we find
\bq
...
Although the exact computational cost will depend heavily on the chosen application, we 
have argued that it is plausible that the augmented system can be solved as efficiently as the original PDE. A summary of the leading order computational costs \emph{per iteration} of the penalty, reduced and all-at-once approaches is given in table \ref{table:costs}. 
