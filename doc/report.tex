...
\title[A penalty method for PDE-constrained optimization in inverse problems]{A penalty method for PDE-constrained optimization}
...
from partial measurements of solutions of a partial differential equation (PDE) that describes the underlying 
physics (e.g., the wave-equation or Maxwell's equation). 
...
A necessary condition for a solution $(\mathbf{m}^*,\mathbf{u}^*,mbf{v}^*)$ to the constrained problem (\ref{eq:constr}) is that 
it is a stationary point of the Lagrangian, i.e. $\nabla\mcl{L}(\mathbf{m}^*,\mathbf{u}^*,mbf{v}^*) = 0$. 
...
nonlinear problem in $\mbf{m}$. To illustrate this, consider the following example.

\begin{example}
Consider the following constrained optimization problem
\[
\min_{m,u} \|u-2\|_2^2 \quad, s.t. \quad  mu = 1.
\]
\[
\mcl{L} \|u-2\|_2^2 + v(mu-1)
\]
\[
\min_{m} \|m^{-1} - 2\|_2^2.
\]
...
for large-scale problems. Our approach is based on the elimination of the state variable $u$ via
...
\STATE{$k=0$}
\REPEAT
\STATE{$\mathbf{u}^k_{\lambda}  = \left(A^TA + \lambda^{-1}PP^T\right)^{-1}\left(A^T\mbf{q} + \lambda^{-1}P^T\mbf{d}\right)$}
...
\label{table:1D_example1a}
\end{table}

\begin{table}
\centering
\input{./figs/2D_example1_a}
...
\label{table:1D_example1b}
\end{table}

% \begin{table}
% \centering
% \input{./figs/1D_example1_b}
...
\centering
\begin{tabular}{ccc}
\includegraphics[scale=.2]{./figs/1D_example1_a}&
\includegraphics[scale=.2]{./figs/1D_example1_b}&
\includegraphics[scale=.2]{./figs/1D_example1_c}\\
% \includegraphics[scale=.2]{./figs/1D_example1_d}&
% \includegraphics[scale=.2]{./figs/1D_example1_e}&
% \includegraphics[scale=.2]{./figs/1D_example1_f}\\
...
\centering
\begin{tabular}{ccc}
\includegraphics[scale=.2]{./figs/2D_example1_a}&
\includegraphics[scale=.2]{./figs/2D_example1_b}&
\includegraphics[scale=.2]{./figs/2D_example1_c}\\
% \includegraphics[scale=.2]{./figs/1D_example1_d}&
% \includegraphics[scale=.2]{./figs/1D_example1_e}&
% \includegraphics[scale=.2]{./figs/1D_example1_f}\\
{\small $L = 1$}&{\small $L = 10$}&{\small $L = 20$}\\
