...
\vspace{1mm}
\STATE{$\mathbf{v}^{0}_{\mathrm{red}}  = A(\mathbf{m}^{0})^{-T}P^T\!(\mathbf{d} - P\mathbf{u}^{0}_{\mathrm{red}})$}
\vspace{1mm}
...
\vspace{2mm}
\STATE{$\mathbf{v}^{k+1}_{\mathrm{red}}  = A(\mathbf{m}^{k+1})^{-T}P^T\!(\mathbf{d} - P\mathbf{u}^{k+1}_{\mathrm{red}})$}
\vspace{1mm}
\STATE{$k = k + 1$}
\ENDWHILE
\end{algorithmic}
...
\mcl{P}_{\mbf{u},\mbf{u}} &=&P^T\!P + \lambda A(\mbf{m})^T\!A(\mbf{m}),\\
\label{eq:Hl}
...
\label{eq:hesspen}
\nabla^2\phi_{\lambda}(\mbf{m}) &=& \mcl{P}_{\mbf{m},\mbf{m}}(\mbf{m},{\mbf{u}}_{\lambda}) \nonumber\\
...
\STATE{$k=0$}
\STATE{$\mathbf{u}^{0}_{\lambda}  = \left(A(\mathbf{m}^{0})^TA(\mathbf{m}^{0}) + \lambda^{-1}P^T\!P\right)^{-1}\left(A(\mathbf{m}^{0})^T\mbf{q} + \lambda^{-1}P^T\!\mbf{d}\right)$}
...
\vspace{1mm}
\STATE{$\mathbf{u}^{k+1}_{\lambda}  = \left(A(\mathbf{m}^{k+1})^TA(\mathbf{m}^{k+1}) + \lambda^{-1}P^T\!P\right)^{-1}\left(A(\mathbf{m}^{k+1})^T\mbf{q} + \lambda^{-1}P^T\!\mbf{d}\right)$}
\vspace{2mm}
...
for any $\mbf{m}$
\bq
\mcl{L}_{\mathbf{u}}(\mbf{m},\mbf{u}_{\lambda},\mbf{v}_{\lambda}) &=& A(\mbf{m})^T\mbf{v}_{\lambda} + P^T\!(P\mbf{u}_{\lambda} - \mbf{d})\nonumber\\
&=& \lambda A^T(A\mbf{u}_{\lambda} - \mbf{q}) + P^T\!(P\mbf{u}_{\lambda} - \mbf{d}) = 0.
\eq
