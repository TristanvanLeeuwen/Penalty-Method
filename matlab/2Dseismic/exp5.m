...
mfun = @(zz,xx)1./(c0 + 1e-3*0.7*zz(:) + (c1-c0)*exp(-1e-3*(xx(:)-3000).^2 - 5e-5*(zz(:)-2100).^2) + (c2-c0)*exp(-1e-3*(xx(:)-7000).^2 - 5e-5*(zz(:)-1800).^2)).^2;mfun = @(zz,xx)1./(c0 + 1e-3*0.75*zz(:) + .5*exp(-1e-7*(xx(:)-3000).^2 - 1e-6*(zz(:)-2100).^2) + 1*exp(-1e-6*(xx(:)-7000).^2 - 1e-6*(zz(:)-1800).^2)).^2;
...
plot2 = @(m)imagesc(x,z,reshape(1./sqrt(m),n),[2 5.5]);
